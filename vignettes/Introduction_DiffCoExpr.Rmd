...
``` r
geneBCMatrixPath <- system.file("extdata", "filtered_gene_bc_matrices", package = "DiffCoExpr")
cellTypesPath <- system.file("extdata", "cellTypes.csv", package = "DiffCoExpr")
srat <- prepareData(geneMatrixPath = geneBCMatrixPath, cellTypesPath = cellTypesPath)
```
...
We can limit our data to just one cell type. Here we are only interested in constructing the matrices for B cells and the matrices for . To create expression and correlation matrices:
``` r
